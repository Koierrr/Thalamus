...
/**
 * Reverse lookup: which workspace contains this session?
 * `workspacesTable` is the parsed `tables.workspaces` map from
 * ~/.dsh/storages/workspace.json ({ <wsId>: { sessionIds: [...] } }).
 * Returns the workspaceId or null (unknown/unaffiliated sessions are never
 * treated as muted — fail-open).
 */
export function findWorkspaceIdForSession(sessionId, workspacesTable) {
	for (const [wsId, row] of Object.entries(workspacesTable || {})) {
		if (Array.isArray(row?.sessionIds) && row.sessionIds.includes(sessionId)) return wsId;
	}
	return null;
}

/** Mute-list membership; entries may be {id,title} objects or bare ids. */
export function isWorkspaceMuted(workspaceId, muted) {
	if (!workspaceId) return false;
	return (muted || []).some((w) => (typeof w === "string" ? w : w?.id) === workspaceId);
}
