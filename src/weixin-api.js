...
const CHANNEL_VERSION = 'dsh-wechat-companion/0.1';
...
async function weixinRequest(creds, endpoint, body, timeoutMs = API_TIMEOUT_MS, routeTag, { ignoreBusinessError = false } = {}) {
  ...
  // The ilink API reports business-level failures INSIDE an HTTP-200 body,
  // in TWO different shapes: `{errcode:-14, errmsg}` (session/token expiry)
  // and `{ret:-2, errmsg:"prepare failed"}` (send-plane rejections). Both
  // previously passed as success and silently dropped proactive sends —
  // surface either as a real error so callers can react and log.
  // Exception: getupdates treats errcode as part of its normal contract
  // (the poll loop pauses on -14), so it opts out via ignoreBusinessError.
  const bizCode = typeof parsed?.errcode === 'number' ? parsed.errcode
    : typeof parsed?.ret === 'number' ? parsed.ret
    : undefined;
  if (!ignoreBusinessError && bizCode !== undefined && bizCode !== 0) {
    throw new Error(`WeChat API ${endpoint} failed: code=${bizCode} ${parsed.errmsg || ''}`.trim());
  }
  return parsed;
}
...
export async function getUpdates(creds, getUpdatesBuf, timeoutMs = LONG_POLL_TIMEOUT_MS) {
  try {
    return await weixinRequest(
      ...
      timeoutMs + 5_000,
      undefined,
      // errcode (-14 session expiry) is part of the long-poll contract — the
      // poll loop inspects it and pauses; it must arrive as data, not a throw.
