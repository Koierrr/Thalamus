...
/** 跨平台解析 .dsh 目录（Windows 无 HOME，用 USERPROFILE/os.homedir 兜底） */
function resolveDshHome() {
  return process.env.DSH_HOME || path.join(process.env.USERPROFILE || process.env.HOME || os.homedir(), '.dsh');
}
...
const CURRENT_DIR_NAME = 'wechat-companion';
...
export class Store {
  ...
  _load() {
    ...
    if (!this._cache.contextTokenAt) this._cache.contextTokenAt = {};
  ...
  upsertContextToken(accountId, peerUserId, token) {
    ...
    // Record WHEN the token was refreshed: the server-side session behind it
    // expires after a long silence (errcode -14), and the Settings tab shows
    // this age so staleness is visible before pushes start failing.
    if (!d.contextTokenAt[accountId]) d.contextTokenAt[accountId] = {};
    d.contextTokenAt[accountId][peerUserId] = Date.now();
    this._save();
  }
  /** Epoch ms of the last context-token refresh for one peer, if any. */
  contextTokenAt(accountId, peerUserId) {
    return this._load().contextTokenAt[accountId]?.[peerUserId];
  }
  deleteContextTokensByAccount(accountId) {
    const d = this._load();
    delete d.contextTokens[accountId];
    delete d.contextTokenAt[accountId];
    this._save();
  }
  /**
   ...
   */
  /** 某个账号下所有"给她发过消息的人"（有会话令牌才发得出去） */
  listContextPeers(accountId) {
    const d = this._load();
    return Object.keys((d.contextTokens || {})[accountId] || {});
  }
